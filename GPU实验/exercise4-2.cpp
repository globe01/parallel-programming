    ...
    auto e = q.submit([&](sycl::handler& h) {
        h.parallel_for<class k_name_t>(
            sycl::nd_range<2>(global_ndrange, local_ndrange), [=](sycl::nd_item<2> index) {
                ...
                float subA[tileY][increase] = { 0.0f };
                float subB[increase][tileX] = { 0.0f };

                for (int k = 0; k < N; k += increase) {
                    for (int m = 0; m < tileY; m++)
                        for (int j = 0; j < increase; j++)
                            subA[m][j] = A[(row + m) * N + k + j];
                    for (int j = 0; j < increase; j++)
                        for (int p = 0; p < tileX; p++)
                            subB[j][p] = B[(k + j) * N + p + col];
                    for (int m = 0; m < tileY; m++)
                        for (int j = 0; j < increase; j++)
                            for (int p = 0; p < tileX; p++)
                                sum[m][p] += subA[m][j] * subB[j][p];
                }
                for (int m = 0; m < tileY; m++)
                    for (int p = 0; p < tileX; p++)
...
int verify(float* cpu_res, float* gpu_res, int increase) {
    int err = 0;
